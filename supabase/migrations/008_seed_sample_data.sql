-- DEV-ONLY SEED: runs only if the `dev` schema exists (created by 009_dev_seed_tools)
DO $$
BEGIN
  IF to_regnamespace('dev') IS NULL THEN
    RAISE NOTICE 'dev schema missing — skipping 008_seed_sample_data (dev-only)';
    RETURN;
  END IF;

  -- pricing_tiers: add any missing columns used by the seed (safe if already present)
  ...
  -- availabilities: support both naming styles + required fields
