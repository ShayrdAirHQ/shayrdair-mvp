...
-- Purpose: Lightweight, realistic seed data for DEV to support end-to-end flows
-- Notes:
-- - Uses fixed UUIDs so re-runs do not duplicate
-- - Uses ON CONFLICT (id) DO UPDATE to be idempotent
-- - Assumes PK on id for all tables and existing FKs per your schema

BEGIN;

-- =========================
-- Users (guide, customer, admin/test)
-- =========================
...
-- =========================
-- Guides (linked to guide user)
-- =========================
INSERT INTO public.guides (id, user_id, bio, profile_image_url, created_at)
VALUES
  ('44444444-4444-4444-4444-444444444444',
   '11111111-1111-1111-1111-111111111111',
   'AMGA Single-Pitch Instructor with 10+ years guiding in Clear Creek and Eldorado Canyon. Patient, safety-first, stoked on helping climbers progress.',
   'https://images.example.com/guides/alex-rivera.jpg',
   now())
ON CONFLICT (id) DO UPDATE
...
-- =========================
-- One Experience (linked to guide)
-- =========================
INSERT INTO public.experiences (
  id, guide_id, created_at,
  title, slug, short_description, description,
  location_name, location_lat, location_lng,
  duration_label, duration_hours,
  trip_type, difficulty_level, season,
  max_group_size,
  price_base, price_step, price_minimum,
...
VALUES (
  ...
  'Half Day', 4,
  'single-day', 'intermediate', 'spring,summer,fall',
  6,
  225.00, 25.00, 125.00,
  'https://images.example.com/experiences/ccc-hero.jpg',
  ARRAY[
    'https://images.example.com/experiences/ccc-1.jpg',
    'https://images.example.com/experiences/ccc-2.jpg'
  ],
  NULL,
  'published', FALSE,
  'Seed: DEV only. Use for integration tests.',
...
-- =========================
-- Pricing Tiers (for the experience)
-- =========================
INSERT INTO public.pricing_tiers (id, experience_id, min_group_size, max_group_size, price, label, created_at)
VALUES
...
-- =========================
-- Availabilities (for the experience)
-- =========================
INSERT INTO public.availabilities (id, experience_id, start_at, end_at, slots_total, slots_booked, status, created_at)
VALUES
  ('77777777-7777-7777-7777-777777777771', '55555555-5555-5555-5555-555555555555',
    (now() + INTERVAL '7 days')::timestamptz,  (now() + INTERVAL '7 days'  + INTERVAL '4 hours')::timestamptz,
    6, 0, 'open', now()),
  ('77777777-7777-7777-7777-777777777772', '55555555-5555-5555-5555-555555555555',
    (now() + INTERVAL '14 days')::timestamptz, (now() + INTERVAL '14 days' + INTERVAL '4 hours')::timestamptz,
    6, 2, 'open', now())
ON CONFLICT (id) DO UPDATE
SET experience_id = EXCLUDED.experience_id,
...
-- =========================
-- Conversation (guide + customer)
-- =========================
INSERT INTO public.conversations (
  id, created_at, customer_user_id, guide_user_id, experience_id, last_message_at, status
) VALUES (
...
-- =========================
-- Messages (2 messages in that conversation)
-- =========================
...
VALUES
  ('99999999-9999-9999-9999-999999999991',
   '88888888-8888-8888-8888-888888888888',
   '22222222-2222-2222-2222-222222222222',
   'Hey Alex! Stoked to book this half-day—do you think it''s suitable if I''m comfortably leading 5.9?',
   now()),
  ('99999999-9999-9999-9999-999999999992',
   '88888888-8888-8888-8888-888888888888',
   '11111111-1111-1111-1111-111111111111',
   'Absolutely! We''ll tailor objectives to your level and focus on safe progression. Morning or afternoon work better?',
   now())
ON CONFLICT (id) DO UPDATE
...
-- =========================
INSERT INTO public.reviews (id, experience_id, author_user_id, rating, title, body, created_at)
VALUES
  ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
   ...
   'Alex delivered an awesome, safety-focused half day. Clear coaching on clipping and cleaning anchors—left feeling confident to push into 5.10.',
   now())
ON CONFLICT (id) DO UPDATE
SET experience_id  = EXCLUDED.experience_id,
...
-- =========================
-- Housekeeping: ensure conversation timestamps stay coherent
-- =========================
